...
namespace
{
    constexpr const char* const questnames[] = {"UW - Chamber", "UW - Wastes", "UW - UWG", "UW - Mnt", "UW - Pits", "UW - Planes", "UW - Pools", "UW - Escort", "UW - Restore", "UW - Vale", "FoW - Defend", "FoW - Army Of Darkness", "FoW - WailingLord",
        "FoW - Griffons", "FoW - Slaves", "FoW - Restore", "FoW - Hunt", "FoW - Forgemaster", "FoW - Tos", "FoW - Toc", "FoW - Khobay", "DoA - Gloom 1: Deathbringer Company", "DoA - Gloom 2: The Rifts Between Us", "DoA - Gloom 3: To The Rescue",
...
void DialogsWindow::Draw(IDirect3DDevice9* pDevice)
{
    ...
    auto DialogButton = [](int x_idx, int x_qty, const char* text, const char* help, uint32_t dialog_id) -> void {
        ...
        float w = (ImGui::GetWindowContentRegionMax().x - ImGui::GetWindowContentRegionMin().x - ImGui::GetStyle().ItemInnerSpacing.x * (x_qty - 1)) / x_qty;
        ...
        if (show_favorites) {
            for (auto i = 0; i < fav_count; ++i) {
                ...
                if (ImGui::Button("Send", ImVec2(60.0f, 0))) {
                    GoNPCSendDialogs({QuestAcceptDialog(IndexToQuestID(fav_index[i])), QuestRewardDialog(IndexToQuestID(fav_index[i]))});
                }
                ImGui::PopID();
            }
            ImGui::Separator();
