...
std::vector<std::pair<GW::UI::ControlAction, GuiUtils::EncString*>> HotkeyGWKey::control_labels = {
    ...
    {GW::UI::ControlAction::ControlAction_DropItem, nullptr}, {GW::UI::ControlAction::ControlAction_Follow, nullptr},
...
TBHotkey* TBHotkey::HotkeyFactory(CSimpleIni* ini, const char* section) {
    ...
    if (type == HotkeyToggle::IniSection() && HotkeyToggle::IsValid(ini, section)) {
...
TBHotkey::TBHotkey(CSimpleIni* ini, const char* section)
    ...
    if (ini) {
        ...
        in_range_of_distance =
            static_cast<float>(ini->GetDoubleValue(section, VAR_NAME(in_range_of_distance), in_range_of_distance));
...
bool TBHotkey::IsInRangeOfNPC() {
    ...
    for (const auto agent : *agents) {
        ...
        if (living->login_number || living->player_number != static_cast<uint16_t>(in_range_of_npc_id)) continue;
            ...
            if (GW::SkillbarMgr::GetPlayerSkillbar()->skills[slot].recharge == 0) {
                GW::GameThread::Enqueue([slot]() -> void {
                    GW::SkillbarMgr::UseSkill(
                        slot, GW::Agents::GetTargetId(), static_cast<uint32_t>(ImGui::IsKeyDown(ImGuiKey_ModCtrl)));
                });
            }
        }
    }
